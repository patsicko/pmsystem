...
while(result.next()){
	...
	String projectName=result.getString("");
...
<td><%=status %></td>
<td>
  <a class="btn btn-primary" href="editProject?id=<%=projectId %>>">
    Edit
  </a>
</td>

<td class="action">
  <button class="btn btn-danger" onclick="deleteProject(<%= result.getInt("projectId") %>);">Delete</button>
</td>
...
<div class="updateProjectForm" style="display: none;">
  <form class="login-form" action="UpdateProjectServlet" method="post">
    <h2>Edit Project</h2>
    <div class="form-group">
      <label for="updateProjectName">Project Title</label>
      <input type="text" id="updateProjectName" name="projectName" placeholder="Enter your Project Title" required>
    </div>
    <div class="form-group">
      <label for="updateDescription">Description</label>
      <textarea id="updateDescription" name="description" placeholder="Enter project description" required></textarea>
    </div>
    <div class="form-group">
  ...
  <script>
    ...
    function showUpdateProjectForm() {
        ...
        document.querySelector('.projectTable').style.display = 'none';
        document.querySelector('.projectForm').style.display = 'none';
        document.querySelector('.updateProjectForm').style.display = 'block';
      }
  ...
  function editProject(projectId, status) {
	  if (!projectId) {
		  ...
		  if (!row) {
		    console.error(`Element with data-project-id="${projectId}" not found.`);
		    return;
		  }

	  const projectName = row.querySelector('td:nth-child(2)').innerText;
	  const description = row.querySelector('td:nth-child(3)').innerText;
	  const user = row.querySelector('td:nth-child(4)').innerText;

	  const projectNameInput = document.getElementById('updateProjectName');
	  const descriptionInput = document.getElementById('updateDescription');
	  const userSelect = document.getElementById('updateUsers');
	  ...
	  statusInput.value = status; // Update the status input value
	  projectIdInput.value = projectId; // Set the projectId in a hidden input field

	  // Show the updateProjectForm
...
<script>
  function deleteProject(projectId) {
    if (confirm("Are you sure you want to delete this project?")) {
      $.ajax({
        url: '/pmsystem/DeleteProjectServlet',
        type: 'POST',
